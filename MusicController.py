...
class MusicController:
    ...
    def stateEntered(self, state, event):
        ...
        if state == 0:
            ...
            self._lightstrip.setPixel(4, BLUE)
        elif state == 5:
            # entry actions for state 1
            self._buzzer.play(self._instrument.getNote(4))
            self._mydisplay.showText("SO")
            self._lightstrip.setPixel(5, PURPLE)
        elif state == 6:
            # entry actions for state 1
            self._buzzer.play(self._instrument.getNote(5))
            self._mydisplay.showText("LA")
            self._lightstrip.setPixel(6, ORANGE)
        elif state == 7:
            # entry actions for state 1
            self._buzzer.play(self._instrument.getNote(6))
            self._mydisplay.showText("TI")
            self._lightstrip.setPixel(7, GREEN)
        elif state == 8:
            # entry actions for state 1
            self._buzzer.play(self._instrument.getNote(7))
            self._mydisplay.showText("DO")
            self._lightstrip.setPixel(8, CYAN)
